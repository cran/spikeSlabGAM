...
#' @param betaInd (optional) the column indices of the part of the design matrix for which the linear predictor is to be evaluated.
#' @return a matrix containing the samples of the linear predictor associated with \code{label}; with attribute \code{'coef'} that
#' 	contains the posterior samples of the associated coefficients. 
#' @author Fabian Scheipl 
#' @export
getPosteriorTerm <- function(label=NULL, model, betaInd=NULL){
	stopifnot(any(c(!is.null(label), !is.null(betaInd))), class(model)=="spikeSlabGAM")
	if(is.null(betaInd)){
		betaInd <- which(model$model$groupIndicatorsOrig==label)
	} 
	
	beta <- t(do.call(rbind, 
					lapply(model$samples$beta,"[", i=1:model$mcmc$chainLength, j=betaInd, drop=F)))
...
#' @param model  a \code{spikeSlabGAM} object
#' @param newdata \code{data.frame} on which to evaluate \code{label}. 
#' 			Defaults to NULL, in which case the term is evaluated on the original data.
#' @param aggregate (function) a summary statistic that is applied over the mcmc-samples of the linear predictor. 
#' 			Defaults to \code{mean}. 
#' @param quantiles (numeric) a vector of quantiles for borders of credible regions of the linear predictor. 
#' 			Defaults to 10 and 90 percent quantiles, i.e. a (point-wise) 80 percent credible region.
#' @param returnData  should the relevant original variables be included in the returned \code{data.frame}? Defaults to TRUE.
#' @return A \code{data.frame} that contains the relevant variables from newdata (if \code{returnData} is TRUE), 
#' 			the \code{aggregate}-summary and the requested \code{quantiles}.  
#' @author Fabian Scheipl 
#' @export
evalTerm <- function(label, model, newdata=NULL, aggregate=mean, quantiles=c(.1,.9), returnData=TRUE){
	...
	if(!is.null(newdata)){
		...
		coef <- if(!is.null(model$predvars[[label]]$qrB)){
					#posterior of coefficients for un-orthogonalized interactions:
					# idea: 
					# 1. B is the design for the interaction BEFORE low-rank approx. (saved in predvars)
					# 2. fhat is the posterior distribution of B%*%coef (sampled via B_loRank %*% coef_loRank) 
					# --> get coefs for original design matrix B via qr(B) : R beta = Q' fhat
					tmp <- try(qr.coef(model$predvars[[label]]$qrB, fhat), silent = TRUE)
					if(class(tmp)=="try-error"){
						tmp <- (ginv(qr.R(model$predvars[[label]]$qrB))%*%t(qr.Q(model$predvars[[label]]$qrB)))%*%fhat
					} 
					tmp
				} else {
					attr(fhat, "coef")
		...
		if(any(is.na(coef))) {
			##FIXME
			warning("Recalculating original coefficients for ", label," seems to be problematic. Predictions may be unstable." )
			rm <- is.na(coef[,1])
			#coef <- coef[!rm, , drop=F]
			#X <- X[, !rm, drop=F]
			coef[rm, ] <- 0
...
#' @param newdata an optional \code{data.frame} on which to evaluate predictions. 
#' 	Defaults to NULL, in which case the fitted values for the original data are returned
#' @param type 	the type of prediction required. The default is on the scale of response, 
...
#' @param terms an optional character vector of term labels or variable names for which to return fits/predictions/credible regions.
...
#' @param aggregate (function) the summary statistic of the posterior predictive of the linear predictor. 
#' 			Defaults to \code{mean}. 
#' @param quantiles (numeric) an optional vector of quantiles for borders of credible regions of the returned values. 
#' 			Defaults to NULL.
#' @param addIntercept include global intercept term in prediction/estimate? Defaults to TRUE if \code{terms=NULL}. 
	...
	if(is.null(newdata)){
		if(type!="terms"){
			...
			terms <- if(is.null(terms)){
						...
						which <- unique(unlist(sapply(terms[!specials], grep, x=names(object$predvars), fixed=TRUE)))
		...
		res <- lapply(terms, evalTerm, model=object, newdata=newdata, aggregate=aggregate, quantiles=quantiles, returnData=FALSE)
		if(type=="terms"){
			names(res) <- terms
			if(attr(terms(object$formula),"intercept")!=0 & addIntercept){
				int <- getPosteriorTerm(model=object, betaInd=1)
				int <- summarizeF(int, aggregate=aggregate, quantiles=quantiles)
				res[["(Intercept)"]] <- suppressWarnings(matrix(int, nrow=nrow(newdata), ncol=ncol(int), byrow=T))
			}
			return(res)
		} else {
			if(attr(terms(object$formula),"intercept")!=0 & addIntercept){
