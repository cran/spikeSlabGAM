...
centerBase <- function(B, x, degree = 0, method = c("P", 
				"QR")) {
	...
	if (degree == 0 && method == "QR") {
		...
		# B_centered[,i] = resid(lm(B[,i] ~ 1 + x + ... + x^degree)) bzw resid(lm(B[,i] ~ -1 + Xx))
...
iwls.start <- function(X, y, family, weights = rep(1,n), offset=rep(0,n), steps=5)
{
	n <- length(y)
	p <- NCOL(X)
	if(family == 2){
...
mmDesign <- function(B, K, tol=1e-10, rankZ=.995)
{
	...
	colsZ <- if(rankZ < 1){
				#use at least 3 but at most so many eigenvectors that rankZ*100% of info in K is represented
				max(3, min(ncol(B), min(which( cumsum(ek$values[!nullvals])/sum(ek$values[!nullvals]) >= rankZ ))))
			} else {
				rankZ
			}
		...
		C=B%*%(Cov%*%t(B)), tol=1e-10, rankZ=.995, rank=NULL)
{
	...
	colsZ <- if(rankZ < 1){
				max(3, min(which( cumsum(eC$values[!nullvals])/sum(eC$values[!nullvals]) > rankZ )))
			} else {
				rankZ
			}
...
psBasis <- function(x, K=length(x), spline.degree=3, diff.ord=2, knots=NULL){
	if(is.null(knots)){
	...
	return(list(X=X, P=P, knots=knots, K=K, spline.degree=spline.degree, diff.ord=diff.ord))
} 
...
#nu 	Number of desired left singular vectors
#nv 	Number of desired right singular vectors
#adjust	Number of extra approximate singular values to compute to enhance convergence
...
#tol 	Convergence is determined when ||A*V - U*S|| <= tol*||A||, where ||A|| is approximated by the largest singular value of all projection matrices.
...
#       Compute block diagonal matrix 
				...
				if (interchange) W[,j+1] <- t (as.matrix(crossprod (V[,j+1, drop=FALSE],A)))
...
#     Update and compute the k x k+1 part of B
			UT <- R[1:k+1, 1:k, drop=FALSE] + R[,k+1,drop=FALSE] %*% t(V_B_last)
...
# ---------------------------------------------------------------------
	...
	#return ( list(d=Bsvd$d[1:k_org], u=W[, 1:(dim(Bsvd$u)[1]), drop=FALSE] %*% Bsvd$u[,1:k_org, drop=FALSE], v=V[,1:(dim(Bsvd$v)[1]), drop=FALSE] %*% Bsvd$v[,1:k_org, drop=FALSE],iter=iter))
	#DPT. OF DIRTY TRICKS: modified names to conform with those returned by eigen to save some keystrokes/assignments in orthoDesign() ...
	return ( list(values=Bsvd$d[1:k_org], vectors=W[, 1:(dim(Bsvd$u)[1]), drop=FALSE] %*% Bsvd$u[,1:k_org, drop=FALSE], v=V[,1:(dim(Bsvd$v)[1]), drop=FALSE] %*% Bsvd$v[,1:k_org, drop=FALSE],iter=iter))
}
