...
#' @param diff.ord order of the difference penalty, defaults to 2 for penalizing deviations from linearity
#' @param rankZ how many eigenvectors to retain from the eigen decomposition: either a number > 3 or the proportion of 
#' 		the sum of eigenvalues the retained eigenvectors must represent at least. Defaults to .999. 	   
#' @param centerBase project the basis of the penalized part into the complement of the column space of the
#' 			basis of the unpenalized part? defaults to TRUE 
#' @param centerx vector of x-values used for centering (defaults to \code{x}) 
#' @param decomposition  use a truncated spectral decomposition of the implied prior covariance of \eqn{f(x)} for a low rank representation 
#' 		with orthogonal  
...
sm <- function(x,  
		...
		tol=1e-10){
	base <- psBasis(x=x, K=K, spline.degree=spline.degree, diff.ord=diff.ord)
	if(centerBase){
	...
	B <- switch(match.arg(decomposition),
			ortho = {
				tmp <- orthoDesign(B=base$X, Cov=ginv(base$P), tol=tol, rankZ=rankZ)
				scaleMat(tmp)
			},
			MM = {
	...
	makeNewX <- function(xnew){
		if(any(xnew<rng[1] | xnew>rng[2])) warning("predictions outside fitted range for ",label,".")
		X <- apply(Bu, 2, function(y){
					splinefun(x=xu, y=y, 'natural')(xnew)
				}) 
		return(X)
	}
	makeNewX <- customFunctionEnv(makeNewX, xu=xu, Bu=Bu, rng=rng, label=label)
...
lin <- function(x, order=1){
	...
	makeNewX <- customFunctionEnv(makeNewX, coefs=coefs, scale=scale, order=order, rng=rng, label=label, xu=xu)
...
#' @param contr (character) name of the \code{\link[stats]{contrasts}} associated with the factor. Defaults to \code{"contr.sum"}.
#' @author Fabian Scheipl
...
fct <- function(x, contr="contr.sum"){
	...
	makeNewX <- customFunctionEnv(makeNewX, lvls=lvls, contr=contr, scale=scale, label=label, xu=xu)
	return(structure(B, label=label, predvars=list(makeNewX=makeNewX)))
}
#' Generate design for a random intercept  
#' 
#' @param x a covariate that can be treated as a factor
#' @param C an optional known correlation structure of the random effects associated with x 
#' @author Fabian Scheipl
...
rnd <- function(x, C=NULL){
	...
	makeNewX <- customFunctionEnv(makeNewX, lvls=lvls, cC=cC, xu=lvlMap$lvl, scale=scale, label=label)
	return(structure(B, label=label, predvars=list(makeNewX=makeNewX)))
}
...
u <- function(x, ...){
	...
	makeNewX <- customFunctionEnv(makeNewX, rng=rng, isFctr=isFctr, label=label, xu=xu)
	return(structure(B, label=label, predvars=list(makeNewX=makeNewX)))
}
#' Generate design for a 2-D Gaussian Markov Random Field 
#' 
#' The returned design is (a low-rank approximation to) the matrix square root of the implied covariance of the centered MRF.
#' The function stops if 'islands', i.e. regions without any neighbors are found. 
#' Regions without observations have to be removed from the neighborhood matrix and there is currently no \code{predict}-functionality for regions
#' without observations in the original data.
#'  
#' 
#' @param x a factor: which observation belongs to which region 
#' @param N the neighborhood (adjacency) matrix: a symmetric matrix with one column/row for every level of \code{x}, defining the  
#' 		neighborhood structure (either 0-1 or with positive weights, e.g. based on shared boundary length or centroid distances).
#' 		Has to have rownames and column names that correspond to the levels of \code{x},  	
#'  	the function checks whether the rows/columns are in the same order as the levels of \code{x}. Entries on the diagonal are ignored. 
#' @param decomposition  use a (truncated) spectral decomposition of the implied prior covariance of \eqn{f(x)} for a low rank representation 
#' 		with orthogonal  basis functions and i.i.d. coefficients (\code{"ortho"}), or use the mixed model reparameterization for 
#'      non-orthogonal basis functions  and i.i.d. coefficients (\code{"MM"}). Defaults to \code{"MM"}.    
...
#' @references Fahrmeir, L., Lang, S. (2001)
#' 	Bayesian inference for generalized additive mixed models based on Markov random field priors. 
#' 	\emph{Applied Statistics}, \bold{50}(2):201--220.
#' @export 
mrf <- function(x, N, decomposition=c("ortho", "MM"), tol=1e-10, rankZ=.995){
	...
	makeNewX <- customFunctionEnv(makeNewX, lvls=lvls, lvlMap=lvlMap, xu=lvlMap$lvl, label=label, nc=NCOL(B))
	return(structure(B, label=label, predvars=list(makeNewX =makeNewX)))
}

#' Generate design for penalized surface estimation.
#' 
#' This function generates the design for a 2-D penalized spline using (almost) radial basis functions. 
#' Use this type of term to account for \emph{spatial} variation. Smooth interactions between covariates are often better fitted via the
#' interactions of \code{\link{lin}} and \code{\link{sm}} terms, because they allow a decomposition
#' into (linear and smooth) marginal trends and (linear-linear, linear-smooth/"varying coefficients", and smooth-smooth) interactions.
#' This decomposition usually makes no sense for spatial data. 
#' 
#' Note that \code{srf()} expects \code{coords} to be a \code{data.frame} within the larger \code{data.frame} supplied to 
#' 	\code{\link{spikeSlabGAM}} in its \code{data} argument, i.e. \code{coords} is considered a two-dimensional covariate. 
#'  
#'  If \code{baseType} is \code{'thinPlate'}, knot locations for the thin plate spline basis are chosen via a space-filling 
#'  algorithm (i.e. medoids returned by \code{\link[cluster]{clara}}) as suggested in Ruppert/Wand/Carroll, ch. 13.5.
#'  Since the thin plate penalty term penalizes deviations from a linear trend, it is recommended to add marginal linear trends 
#'  and their interaction to the model if \code{baseType="thinPlate"} to improve the fit. 
#' 
#' @note TODO: prediction seems very unstable when extrapolating outside the convex hull, 
#' 	sometimes even nonsensical values inside conv.hull close to border - problem in akima::interp?  
#' 
#' @param coords a \code{data.frame} with two columns containing the coordinates
#' @param K (approximate) number of basis functions in the original basis (defaults to 50). 
#'  If \code{baseType="B"} you can specify a vector giving the number of marginal basis functions in each direction. 
#' @param rankZ how many eigenvectors to retain from the eigen decomposition: either a number > 3 or the proportion of 
#' 		the sum of eigenvalues the retained eigenvectors must represent at least. Defaults to .999. 	   
#' @param centerBase project the basis of the penalized part into the complement of the column space of the
#' 			basis of the unpenalized part? defaults to TRUE 
#' @param baseType Defaults to \code{"B"}, i.e. a tensor product basis based on marginal cubic B-splines with ridge penalty (i.e. penalizing deviations from the constant). 
#'  Set to \code{"thinPlate"} if cubic thin plate splines are desired, see note below. 
#' @param decomposition  use a (truncated) spectral decomposition of the implied prior covariance of \eqn{f(x,y)} for a low rank representation 
#' 		with orthogonal	basis functions and i.i.d. coefficients (\code{"ortho"}), or use the mixed model reparameterization for non-orthogonal
#' 		basis functions 
#'      and i.i.d. coefficients (\code{"MM"}) or use basis functions as they are with i.i.d. coefficients (\code{"asIs"}). Defaults to \code{"ortho"}.
#' @param tol count eigenvalues smaller than this as zero 
#' @author Fabian Scheipl
...
srf <- function(coords, K=min(50, sum(nd)/4), rankZ=.999, centerBase=TRUE, baseType=c("B", "thinPlate"),
	...
	if(baseType == "thinPlate"){
		if(length(K) > 1) warning("Only using first element of K for thin plate basis.")
	...
	makeNewX <- function(xnew){
		if(any(!insidePoly(xnew, rng))) {
			warning("Prediction locations outside of convex hull of original observations for ", label, ".")
		}	
			
		X <- apply(Bu, 2, function(b){
					interpp(x=xu[,1], y=xu[,2], z=b, xo=xnew[,1], yo=xnew[,2], extrap=FALSE,
							linear=FALSE)$z				
				})
		return(X)
	}
	makeNewX <- customFunctionEnv(makeNewX, xu=coordsu,  Bu=Bu, rng=rng, label=label, insidePoly=insidePoly)
	ret <- structure(B, label=label, rng=rng, predvars= list(makeNewX = makeNewX))
	return(ret)
}
