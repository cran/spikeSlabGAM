...
plotTerm <- function(label, m, cumulative=TRUE, 
	...
	if(any(isSrf)){
		...
		varsLin <- sapply(trms[isLin], function(x) gsub("\\)","",gsub("([[:graph:]]+\\()","", x))) 
		varsSrf <- sapply(trms[isSrf], function(x) gsub("\\)","",gsub("([[:graph:]]+\\()","", x)))
		
		if(any(isLin)){
			# TODO: check degree=1
			if(any(cumulative)){
				warning("Cumulative plot for a space-varying linear effect ", label," makes no sense. Setting cumulative=FALSE.")
				cumulative <- FALSE
			}
		}
		...
		facGrid <- if(any(isFct)){
					do.call(expand.grid, lapply(xu[!dfs & !(names(xu)%in%varsLin)], function(x){
										return(sort(x))
									}))
				} else NULL
		...
		if(NCOL(linGrid)>1) {
				warning("Plots for 3 way numeric interactions with srf() not implemented, returning NULL.")
				return(NULL)
		}	
		...
		# get predictions
		pred <- data.frame(evalTerm(label, model=m, newdata=grid, aggregate=aggregate,
							quantiles=quantiles, returnData=FALSE))	
		fcts <- sapply(grid[names(grid) !="FILLER"], is.factor)
		nums <- sapply(grid[names(grid) !="FILLER"], function(x) !is.factor(x)&!is.data.frame(x))
		...
		if(length(vars)==3 & sum(fcts)==1){
			nms <-  paste(paste(names(xu)[dfs],"$",sep=""), names(srfGrid), sep="")
			nmLin <- gsub("[aeiou]","",varsLin[1])
			linLabel <- parse(text=paste("frac(partialdiff*eta, partialdiff*",nmLin,")"))
			aesStr <- aes_string(x=nms[1], y=nms[2], z=aggregateStr, colour="..level..")
			plotElems <- c(
					facet_grid(eval(substitute(formula(. ~ .a), list(.a=as.name(names(fcts)[fcts])))), labeller=label_both),
					geom_contour(data=data, mapping=aesStr, bins=nBins),
		...
		if(length(vars)==1){
			if(is.factor(grid[,1])){
				...
				plotElems <- c(	geom_line(data=data, aesAggrStr),
						geom_rug(data=data.frame(m$data), aes_string(x=vars,y=NULL), alpha=alphaRug),
						list(ylab(expression(eta))),
						plotElems)
				if(plotCR){
					aesCRStr <- aes_string(x=vars, ymin=quantileStr[1], ymax=quantileStr[2])
			...
			if(length(vars)==2){
				if(any(fcts)){
					if(all(fcts)){
						...
						geom <- if(plotCR){
									aesStr <- aes_string(x=f2, y=aggregateStr, colour=f1, group=f1, ymin=quantileStr[1], ymax=quantileStr[2])
									geom_pointrange(aesStr, position=position_dodge(width=.66))
								} else {
									aesStr <- aes_string(x=f2, y=aggregateStr, colour=f1, group=f1)
						...
						plotElems <- c(geom_line(aesAggrStr), 
								...
								plotElems)
						if(plotCR){
							aesCRStr <- aes_string(x=vars[nums], ymin=quantileStr[1], ymax=quantileStr[2], fill=vars[!nums])
				...
				if(all(fcts)){
					#fct:fct:fct
					geom <- if(plotCR){
								aesStr <- aes_string(x=vars[3], y=aggregateStr, colour=vars[2], group=vars[2], ymin=quantileStr[1], 
										ymax=quantileStr[2])
								geom_pointrange(aesStr, position=position_dodge(width=.66))
							} else {
								aesStr <- aes_string(x=vars[3], y=aggregateStr, colour=vars[2], group=vars[2])
								geom_point(aesStr, position=position_dodge(width=.66))
							} 
					facetFrml <- eval(substitute(formula( ~ .a), list(.a=as.name(vars[1]))))
...
plot.spikeSlabGAM <- function(x, labels=NULL, cumulative=TRUE, commonEtaScale = FALSE,
	...
	if(is.null(labels)){
		if(cumulative){
			...
			varsList <- sapply(terms, 
					function(x){
						sapply(
								unlist(strsplit(gsub("(:?[[:lower:]]+)(\\([[:alnum:]._]+\\))","\\2", x), c("\\)"))),
			...
			for(o in max(order):1){
				...
				if(any(comboOrder==o)){
					...
					#remove all term combinations that are already involved in the current one
	...
	if(!is.null(rows)) stopifnot(length(rows)==length(plotLabels))
	...
	start <- seq(1, nPlots, by=maxPlotsPerPage)
	stop <- c(start[-1]-1, nPlots)
	
	#	if(dev.interactive() & length(start)>1){
	#		devAskNewPage(ask = TRUE)
	#	} 
	##FIXME: not wrking for some reason .... using readline instead
	
	makeLayout <- function(p){
		p1 <- max(1, floor(log2(p))) 
		p2 <- p/p1
		p2 <- ifelse(floor(p2)*p1 >= p, floor(p2), ceiling(p2))
		# more rows than columns (since legends usually take up much horizontal space..)
		layout <- rev(sort(c(p1,p2)))
		if(diff(layout)< (-1)) layout <- c(layout[1]-1,layout[2]+1)
		grid <- expand.grid(1:layout[1], 1:layout[2])
		grid <- grid[1:p,]
		return(grid)
	}
