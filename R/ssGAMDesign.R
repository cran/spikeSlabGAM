...
#' 
#' This function generates the design matrix for a generalized additive (mixed) model, 
#' based on smoothing spline ANOVA-like orthogonal decomposition of the model terms and their interactions. 
#' It parses the formula given to \code{\link{spikeSlabGAM}} to provide all the arguments necessary for the MCMC sampler.  
#' 
#' Setting \code{lowRankInteractions} to FALSE can result in very large models, especially if higher-order interactions 
#' or interactions between terms with lots of parameters are involved. Note that numeric covariates with fewer unique values than  
#' \code{minUniqueValues} are treated as factors unless wrapped in a special argument. 
#' 
#' This function is not meant to be called directly by the user, \code{\link{spikeSlabGAM}} is the user interface. 
#' @param formula the model formula. Follows the usual
#' 	R syntax described in \code{\link[stats]{formula}}. 
#' 	Terms in the formula that are not in the list of specials are automatically assigned 
#'  an appropriate special, i.e. numerical covariates \code{x} are treated as \code{lin(x) + sm(x)}, factors \code{f} are
#' treated as \code{fct(f)} (see \code{specials} argument). See \code{\link{spikeSlabGAM}} for more details. 
#' @param data \code{data.frame} containing all the variables in the function
#' @param specials a vector of the types of possible model terms. These must be implemented as 
#' 	functions generating a design matrix with a label attribute. See also \code{\link{sm}()} for an example.
#'  The documentation for \code{\link{spikeSlabGAM}} contains a list of implemented model term types and usage examples.
...
#' \item{response}{the left hand side of the model formula}
...
ssGAMDesign <-  function(formula, data, 
	...
	replaceRawTerms <- function(formula, specials, decomposition){
		...
		rawLabels <- if(length(unlist(attr(rawTrms, "specials")))){
					whereSpecials <- unique(unlist(apply(
						attr(rawTrms, "factors")[unlist(attr(rawTrms, "specials")),, drop=F] > 0, 1, which)))
		...
		replace1Term <- function(x){
				decompArg <- ifelse(!is.null(decomposition), paste(", decomposition='", decomposition,"'",sep=""), "")
		...
		replaceRawLabels <- function(x){
			...
			if (!is(x, "name")){
				...
				which <- grep(paste("^",as.character(x),"$",sep=""), names(rawLabelReplacements))
			...
			if(length(intActs)){
				...
				for(l in grep("lin(", mainEfs, fixed=T)){
					...
					if(length(x)){
						...
						if(any(nonsense)) tmp <- c(tmp, paste("-",intActs[nonsense],collapse=""))
					}
				}
			}
			tmp
	...
	makeInteractions <- function(term, Designs, center=orthogonalizeInteractions,
		...
		if(orthogonalize && NCOL(B)>1){
			...
			B <- {
				...
				if(ncol(B)>20 & interActRank > 10){
					eC <- try(irlba(B, max(interActRank, sapply(Xs, NCOL)), max(interActRank, sapply(Xs, NCOL)), adjust=0))
				} else {
					sv <- svd(B, nv=0)
				...
				colsZ <- max(3, min(which( cumsum(eC$values[!nullvals])/sum(eC$values[!nullvals]) > .999)))
	...
	X <- cbind(X, do.call(cbind, mainEffectDesigns), do.call(cbind, interactionDesigns))
	grpInds <- unlist(c(grpInds, 
			rep(sapply(mainEffectDesigns, attr, which="label"), sapply(mainEffectDesigns, NCOL)), 
			rep(sapply(interactionDesigns, attr, which="label"), sapply(interactionDesigns, NCOL))))
	...
	makeHierarchy <- function(formula, grpInds){
		...
		diag(H) <- 1
		rownames(H) <- labels
		##TODO: hierarchy for higher order interactions (i.e. which 2-interactions are involved in which 3-interaction missing)
		return(H)
	}
	...
	return(list(response = y, Design=X, groupIndicators = grpInds, H=H, formula=formula,
					predvars=predvars))
}
