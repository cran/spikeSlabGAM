...
<<compile, echo=F, eval=F>>=
setwd("~/RegularBayes/Workspace/spikeSlabGAM/pkg/inst/doc/")
Sweave("UsingSpikeSlabGAM.Rnw")
tools:::texi2dvi("UsingSpikeSlabGAM.tex", pdf=TRUE, clean=TRUE, quiet=FALSE)

@


<<setup, echo=F, results=hide>>=
options(prompt = "R> ", continue = "+  ", width = 70, useFancyQuotes = FALSE, digits=5)
...
\noindent \emph{Note}: An earlier version of this introduction to \pkg{spikeSlabGAM} has been published as \\
\noindent Fabian Scheipl (2011).\\
\noindent \pkg{spikeSlabGAM}: Bayesian Variable Selection, Model Choice and Regularization for Generalized Additive Mixed Models in \proglang{R}.\\
\noindent \emph{Journal of Statistical Software}, \textbf{43}(14), 1--24.
...
\begin{document}
\SweaveOpts{prefix.string=UsingSpikeSlabGAM,eps=FALSE,pdf=TRUE}
...
@
The function \code{spikeSlabGAM} sets up the design matrices, calls the sampler and returns the results: 
...
If a decomposition into marginal and joint effects is irrelevant or inappropriate, 
bivariate smooth terms can alternatively be specified with a \code{srf()}-term. 
...
# roughly the same as: print(sum, printPGamma=F, printModels=T) 
...
probability of \Sexpr{round(sum[[1]]["111110010100000000000000000010010000"],2)}. The correct model 
that additionally includes \code{lin1} (column \Sexpr{which(names(sum[[1]])=="111111010100000000000000000010010000")}, not shown)
has a posterior probability of about \Sexpr{round(sum[[1]]["111111010100000000000000000010010000"],3)}.}
\label{sum12}
\end{figure}
...
#tweak legend size:
p1 <- plot(m, maxPlotsPerPage=12, ggElems=list(opts(legend.key.size = unit(0.01, "npc"),  legend.text = theme_text(size = 7),
                        legend.title=theme_text(size=8, hjust = 0), axis.text.x = theme_text(size=8),
						axis.text.y = theme_text(size=8, hjust=1),
						plot.margin=unit(c(0.5, 0, 0, 0), "lines"))))
# # replace expression(eta) with latex for tikz-rendering: (JSS-version only)
#scGr <- scale_colour_gradient2(name="$\\eta$", low =  muted('darkblue'), mid = 'grey80', high = muted('darkred'))
#p1[[2]] <- p1[[2]] + scGr
#p1[[4]] <- p1[[4]] + scGr
#p1[[6]] <- p1[[6]] + scGr
...
arrangeHere <- function(...) arrange.ggplots(..., rows=as.list(rep(1:4,3)), cols=as.list(rep(1:3, e=4)))
do.call(arrangeHere, p1)
@
\caption{Posterior means and pointwise 80\% credible intervals for \texttt{m}. 
...
Figure \ref{fig:m1} shows the estimated effects for \code{m}.  
...
The fits are quite close to the truth despite the heavy-tailed errors and the many noise terms included in the model. 
Full disclosure: The code used to render Figures \ref{fig:m1Sm1} and \ref{fig:m1Sm2f} is a little more intricate
than the code snippets we show, but the additional code only affects details (font and margin sizes and the arrangement of the 
panels).  
\setkeys{Gin}{width=6in}
\begin{figure}[!htbp]\begin{center}
\tikzset{font=\small}
...
plot(m, labels="sm(sm1)", ggElems=list(geom_line(aes(x=sm1, y=truth), data=trueFsm1, linetype=2)))
@
<<plotm1sm1Prep, echo=FALSE, cache=TRUE>>=
pSm11 <- plot(m, labels="sm(sm1)", ggElems=list(opts(axis.text.x = theme_text(size=9),
				axis.text.y = theme_text(size=9, hjust=1))))
pSm1Sep <-  plot(m, labels=c("lin(sm1)", "sm(sm1)"), cumulative=FALSE, ggElems=list(opts(axis.text.x = theme_text(size=9),
						axis.text.y = theme_text(size=9, hjust=1))), rows=list(1,1), cols=list(1,2)) #, 
# JSS version only:        ggElems=list(list(ylab("$\\eta$"))))
@
<<plotm1sm1, echo=FALSE, fig=TRUE, width=6, height=2, cache=TRUE, external=TRUE>>=
trueFsm1 <-data.frame(truth=fsm1-mean(fsm1), sm1=sm1)
pSm1Cum <- pSm11[[1]] + geom_line(data=trueFsm1, aes(x=sm1, y=truth), lty=2)  + opts(axis.text.x = theme_text(size=9),
		axis.text.y = theme_text(size=9, hjust=1))# + ylab("$\\eta$")
arrange.ggplots(pSm1Sep[[1]], pSm1Sep[[2]], pSm1Cum)
@
\caption{Posterior means and pointwise 80\% credible intervals for $f(\mathtt{sm1})$ in 
\texttt{m}. Left and middle panel show the separate \texttt{lin()}- and \texttt{sm()}-terms returned by the first call to \code{plot}, 
...
plot(m, labels="sm(sm2):fct(f)", ggElems=list(geom_line(aes(x=sm2, y=truth, colour=f), data=trueFsm2f, linetype=2)))
@
<<plotm1sm2f, echo=FALSE, fig=TRUE, width=6, height=3, cache=TRUE, external=TRUE>>=
trueFsm2f <-data.frame(truth=fsm2f-mean(fsm2f), sm2=sm2, f=f)
...
\texttt{m}. True shape of $f(\mathtt{sm2}|\mathtt{f})$ added as dashed line for each level of \texttt{f}.}
\label{fig:m1Sm2f}
\end{center}\end{figure}
...
run 8 parallel chains for an additive main effects model 
(if either \pkg{multicore} \citep{multicore} or \pkg{snow} \citep{snow} are installed, the chains will be run in parallel): 
...
\pkg{mboost}\code{::gamboost} ranks the variables very similarly, based on the relative selection frequencies of
the associated baselearners:
...
(predictive deviance for \code{m0} was \Sexpr{round(dev(pimaDiab[testInd,"diabetes"], pr0),2)}).
...
and constructive review of an earlier draft of this vignette.
