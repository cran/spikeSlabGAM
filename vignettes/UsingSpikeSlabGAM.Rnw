...
options(prompt = "R> ", continue = "+  ", width = 70, useFancyQuotes = FALSE, digits=5, cores=2)
...
\noindent \emph{Note}:  Due to resource limitations on the CRAN server, this vignette is limited to
using only two cores on machines with \pkg{multicore} installed. To increase the number of cores (which speeds up computation significantly),
change \code{cores=2} in the \code{options()} statement in the \code{setup} \proglang{R}-chunk at the very beginning of   
\texttt{UsingSpikeSlabGAM.Rnw} to a higher value. 
}
...
#tweak legend size:
p1 <- plot(m, maxPlotsPerPage=12, ggElems=list(theme(legend.key.size = grid:::unit(0.01, "npc"),  legend.text = element_text(size = 7),
                        legend.title=element_text(size=8, hjust = 0), axis.text.x = element_text(size=8),
...
pSm11 <- plot(m, labels="sm(sm1)", ggElems=list(theme(axis.text.x = element_text(size=9),
				axis.text.y = element_text(size=9, hjust=1))))
pSm1Sep <-  plot(m, labels=c("lin(sm1)", "sm(sm1)"), cumulative=FALSE, ggElems=list(theme(axis.text.x = element_text(size=9),
						axis.text.y = element_text(size=9, hjust=1))), rows=c(1,1), cols=c(1,2)) #, 
# JSS version only:        ggElems=list(list(ylab("$\\eta$"))))
@
<<plotm1sm1, echo=FALSE, fig=TRUE, width=6, height=2, cache=TRUE, external=TRUE>>=
trueFsm1 <-data.frame(truth=fsm1-mean(fsm1), sm1=sm1)
# fiddling around with classes to aoid stupid grid.arrange bug:
attr(pSm11[[1]], "class") <- c("ggplot", "gg")
pSm1Cum <- pSm11[[1]] + geom_line(data=trueFsm1, aes(x=sm1, y=truth), lty=2)  + theme(axis.text.x = element_text(size=9),
		axis.text.y = element_text(size=9, hjust=1))# + ylab("$\\eta$")
attr(pSm1Cum, "class") <- c("ggplot")
